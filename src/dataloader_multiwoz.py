from src.datareader_multiwoz import datareader, PAD_INDEX, domain2slot, y2_set
import torch
...
def get_dataloader(tgt_domain, batch_size, n_samples, data_path, tokenizer, use_dep, use_pos):
    ...
    if n_samples == 0:
        ...
        tgt_domain_to_all_seen_slots.extend(tgt_data["label"][:train_split])
        tgt_domain_to_all_seen_slots = list(set(tgt_domain_to_all_seen_slots))

    if use_dep and use_pos:
        for dm, label, utter, y, utter_dep, utter_pos, coarse_label, fine_label in zip(tgt_data["domain"][val_split:],
                                                                                       tgt_data["label"][val_split:],
                                                                                       tgt_data["utter"][val_split:],
                                                                                       tgt_data["y"][val_split:],
                                                                                       tgt_data["utter_dep"][
                                                                                       val_split:],
                                                                                       tgt_data["utter_pos"][
                                                                                       val_split:],
                                                                                       tgt_data["gold_coarse_label"][
                                                                                       val_split:],
                                                                                       tgt_data["gold_fine_label"][
                                                                                       val_split:]):
            if label in tgt_domain_to_all_seen_slots:
                seen_slot_cnt += 1
                seen_data["domain"].append(dm)
                seen_data["label"].append(label)
                seen_data["utter"].append(utter)
                seen_data["y"].append(y)
                seen_data["utter_dep"].append(utter_dep)
                seen_data["utter_pos"].append(utter_pos)
                seen_data["gold_coarse_label"].append(coarse_label)
                seen_data["gold_fine_label"].append(fine_label)
            else:
                unseen_slot_cnt += 1
                unseen_data["domain"].append(dm)
                unseen_data["label"].append(label)
                unseen_data["utter"].append(utter)
                unseen_data["y"].append(y)
                unseen_data["utter_dep"].append(utter_dep)
                unseen_data["utter_pos"].append(utter_pos)
                unseen_data["gold_coarse_label"].append(coarse_label)
                unseen_data["gold_fine_label"].append(fine_label)
        print("Target Domain: %s; Seen slots: %d; Unseen slots: %d; Total slots: %d" % (
            tgt_domain, seen_slot_cnt, unseen_slot_cnt, unseen_slot_cnt + seen_slot_cnt))

    elif use_dep:
        for dm, label, utter, y, utter_dep, coarse_label, fine_label in zip(tgt_data["domain"][val_split:],
                                                                                       tgt_data["label"][val_split:],
                                                                                       tgt_data["utter"][val_split:],
                                                                                       tgt_data["y"][val_split:],
                                                                                       tgt_data["utter_dep"][
        ...
        for dm, label, utter, y, utter_pos, coarse_label, fine_label in zip(tgt_data["domain"][val_split:],
                                                                                       tgt_data["label"][val_split:],
                                                                                       tgt_data["utter"][val_split:],
                                                                                       tgt_data["y"][val_split:],
                                                                                       tgt_data["utter_pos"][
        ...
        for dm, label, utter, y, coarse_label, fine_label in zip(tgt_data["domain"][val_split:],
                                                                                       tgt_data["label"][val_split:],
                                                                                       tgt_data["utter"][val_split:],
                                                                                       tgt_data["y"][val_split:],
                                                                                       tgt_data["gold_coarse_label"][
    ...
    dataloader_test = DataLoader(dataset=dataset_test, batch_size=len(domain2slot[tgt_domain]), shuffle=False,
                                 collate_fn=collate_fn)
    dataloader_unseen = DataLoader(dataset=dataset_unseen, batch_size=len(domain2slot[tgt_domain]), shuffle=False,
                                   collate_fn=collate_fn)
    dataloader_seen = DataLoader(dataset=dataset_seen, batch_size=len(domain2slot[tgt_domain]), shuffle=False,
                                 collate_fn=collate_fn)
