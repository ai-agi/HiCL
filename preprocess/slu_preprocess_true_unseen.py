...
def generate_seen_and_unseen_data():
    ...
    data_size = {'AddToPlaylist': 2042, 'BookRestaurant': 2073, 'GetWeather': 2100, 'PlayMusic': 2100, 'RateBook': 2056,
     'SearchCreativeWork': 2054, 'SearchScreeningEvent': 2059}

    data, vocab = datareader()
    for tgt_dm in domains:
        seen_cnt, unseen_cnt = 0, 0
        d_size = int(data_size[tgt_dm])
        scale = math.ceil((500/2000)*d_size)

        print("d_size: ", d_size)
        print("scale: ", scale)

        seen_slots_file = open("../data/new_snips/"+tgt_dm+"/seen_slots.txt", encoding='utf-8', mode="w")
        unseen_slots_file = open("../data/new_snips/"+tgt_dm+"/unseen_slots.txt", encoding='utf-8', mode="w")
        tgt_seen_slots = domain_to_all_seen_slots[tgt_dm]

        dm_data = data[tgt_dm]
        # dm_data["utter"][500:] are for the test set
        for utter_idx, labels_idx in zip(dm_data["utter"][scale:], dm_data["y2"][scale:]):
            utter, labels = [], []
            ...
            for item in labels:
                if "-" in item:
                    item = item.split("-")[1]
                    if item in tgt_seen_slots:  # if a slot of utter exist in tgt_seen_slots and it is classified into seen one
                        flag = True
                        break
            if flag:
                seen_cnt += 1
        ...
        # slot filling with BERT(只要utterance 里有一个属于seen slot 这个utterance 就属于seen slor分类)
        # d_size:  2042
        # scale:  511
        # Domain: AddToPlaylist; Seen samples: 1531; Unseen samples: 0; Total samples: 1531
        # d_size:  2073
        # scale:  519
        # Domain: BookRestaurant; Seen samples: 1293; Unseen samples: 261; Total samples: 1554
        # d_size:  2100
        # scale:  525
        # Domain: GetWeather; Seen samples: 1439; Unseen samples: 136; Total samples: 1575
        # d_size:  2100
        # scale:  525
        # Domain: PlayMusic; Seen samples: 1254; Unseen samples: 321; Total samples: 1575
        # d_size:  2056
        # scale:  514
        # Domain: RateBook; Seen samples: 1424; Unseen samples: 118; Total samples: 1542
        # d_size:  2054
        # scale:  514
        # Domain: SearchCreativeWork; Seen samples: 1540; Unseen samples: 0; Total samples: 1540
        # d_size:  2059
        # scale:  515
        # Domain: SearchScreeningEvent; Seen samples: 1039; Unseen samples: 505; Total samples: 1544
